                            ...
                            <table class="table table-striped table-bordered first">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        if($search){
                                            $sql_str = "SELECT TL_ID, TenTL FROM TheLoai $where ORDER BY TL_ID LIMIT $item_per_page Offset $offset";
                                            $result = mysqli_query($conn, $sql_str);
                                            $record = mysqli_query($conn, "SELECT TL_ID, TenTL FROM TheLoai $where ORDER BY TL_ID");
                                        } else {
                                            $sql_str = "SELECT TL_ID, TenTL FROM TheLoai ORDER BY TL_ID LIMIT $item_per_page Offset $offset";
                                            $result = mysqli_query($conn, $sql_str);
                                            $record = mysqli_query($conn,"SELECT TL_ID, TenTL FROM TheLoai ORDER BY TL_ID");
                                        }
